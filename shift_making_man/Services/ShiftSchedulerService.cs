...
namespace shift_making_man.Services
{
    public class ShiftSchedulerService
    {
        ...
        // 追加: GenerateSchedule メソッド
        public Dictionary<int, List<Shift>> GenerateSchedule(List<Employee> employees, List<Shift> shifts)
        {
            DateTime startDate = DateTime.Now; // スケジュール開始日を設定
            int daysInMonth = DateTime.DaysInMonth(startDate.Year, startDate.Month); // その月の日数を取得

            return GenerateMonthlySchedule(employees, shifts, startDate, daysInMonth);
        }
